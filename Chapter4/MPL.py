...
class MPL:
    ...
    def forward(self, inputs):
        ...
        self.hidden = np.insert(self.hidden, 0, -1, axis = 1)
    ...
    def train(self, inputs, targets, max_iterations = 1000):
        ...
        for i in range(max_iterations):
            ...
            if i % 100 == 0: print(f"Iteration: {i} Error: {error}")
            ...
            deltah = self.hidden * self.beta * (1.0 - self.hidden) * np.matmul(deltao, self.hidden_weights.T)

            update_input = self.eta * (np.matmul(inputs.T, deltah[:,:-1])) + self.momentum * update_input
            update_hidden = self.eta * (np.matmul(self.hidden.T, deltao)) + self.momentum * update_hidden
